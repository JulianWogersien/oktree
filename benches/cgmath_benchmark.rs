...
use cgmath::{ Point3, Vector3, bounding::{ AABB3d, BoundingSphere }, raycast::RayCast3d };
...
fn random_rays() -> Vec<RayCast3d<f32>> {
    ...
    for _ in 0..RAY_COUNT {
        ...
        let direction = Vector3::new(x_dir, y_dir, z_dir);
...
fn random_spheres() -> Vec<BoundingSphere<f32>> {
...
fn random_aabbs() -> Vec<AABB3d<f32>> {
    ...
    for _ in 0..RAY_COUNT {
        ...
        let position = Point3::new(x, y, z);
        let size = Vector3::new(x_size, y_size, z_size);
...
fn octree_ray_cast(tree: &Octree<usize, DummyCell<usize>>, rays: &[RayCast3d<f32>]) {
    for ray in rays {
        let _ = tree.ray_cast(ray);
    }
}

fn octree_sphere_intersect(
    tree: &Octree<usize, DummyCell<usize>>,
    spheres: &[BoundingSphere<f32>]
) {
...
fn octree_aabb_intersect(tree: &Octree<usize, DummyCell<usize>>, aabbs: &[AABB3d<f32>]) {
    for aabb in aabbs {
...
fn criterion_benchmark(c: &mut Criterion) {
    ...
    let tree = octree_insert(&points);
    group.bench_function("octree ray cast", |b| { b.iter(|| octree_ray_cast(&tree, &rays)) });
